...
public:
    ...
    void decreaseKey(int, int, struct Edge);
};
...
void PriorityQueue::decreaseKey(int nodeIdx, int weight, struct Edge edge) {
...
void Graph::primJarnikMST() {
    /* for a simple connected undirected edge-weighted graph G */

    PriorityQueue pQueue;

    struct PQNode temp;
    struct adjListNode temps;
    struct Edge tempE;

    int weight[numberNode];

    for(int i=0; i<numberNode; i++) {
        if(i == 0) {
            weight[i] = 0;  // starting node
        } else {
            weight[i] = INFINITY;
        }
    ...
    while(pQueue.isEmpty() == false) {
        ...
        if(temp.nodeIdx != 0) {
            /* update MST except for starting node */
            mst.addEdge(temp.edge);
        }

        for(int i=0; i<array[temp.nodeIdx].getNumberEdge(); i++) {
            temps = array[temp.nodeIdx].getEdge(i);

            if(weight[temps.nodeIdx] > temps.weight) {
                weight[temps.nodeIdx] = temps.weight;

                tempE.u = temp.nodeIdx;
                tempE.v = temps.nodeIdx;
                tempE.weight = temps.weight;

                pQueue.decreaseKey(temps.nodeIdx, temps.weight, tempE);
            }
        }
    }
